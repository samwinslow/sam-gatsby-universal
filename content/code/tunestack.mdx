...
- [Serverless Framework](https://www.serverless.com/) for provisioning & deploying to AWS
  - Used `serverless-plugin-split-stacks` and `serverless-domain-manager` to get around AWS' limits on the size of a REST API deployed with CloudFormation
...
- Redux is useful, but very opinionated and requires a lot of boilerplate. On the plus side, it will force you into an MVC structure and make it easier to debug async code. *(Note: We have been porting our app to [Flutter](https://flutter.dev/) since June, because the provider/context structure is a bit easier to use.)*
...
[Download the App (iOS)](https://tunestack.app.link/awgyZRPUM5) — *If on desktop, will text you a link to download*
